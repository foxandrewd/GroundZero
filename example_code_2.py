    ...
    if move_is_valid(gameboard_state, button_row, button_col):
        ...
        if current_player_has_won(gameboard_state, Player):
            if Player =="X":
                ...
                lab_X_wins['text'] = "  Wins(X) = " + str(num_X_wins)
            else:
                num_O_wins += 1
                print("Num X wins: ", num_X_wins); print("Num O wins: ", num_O_wins); print("Num Draws : ", num_draws);
                messagebox.showinfo(message = "Player O won.  Well done Player O !!")
                lab_O_wins['text'] = "        Wins(O) = " + str(num_O_wins)
        ...
        if check_draw(gameboard_state):
            ...
            lab_Draws['text'] = "        Draws = " + str(num_draws)
            # Clear the Board for the next Game
            newgame_clear_board(gameboard_state)
            newgame_clear_all_buttons_text(buttons)
...
frame_stats.pack(side=tk.BOTTOM)

lab_X_wins = tk.Label(frame_stats, text="  Wins(X) = " + str(num_X_wins))
lab_O_wins = tk.Label(frame_stats, text="        Wins(O) = " + str(num_O_wins))
lab_Draws = tk.Label(frame_stats, text="        Draws = " + str(num_draws))

lab_X_wins['font'] = statsFont
lab_O_wins['font'] = statsFont
lab_Draws['font'] = statsFont


lab_X_wins.grid(row=0, column=0)
lab_O_wins.grid(row=0, column=1)
lab_Draws.grid(row=0, column=2)
